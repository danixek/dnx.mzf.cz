...
<head>
    ...
    <link rel="shortcut icon" href="/DEVportfolio/logo/dnx-logo_mini.ico" type="image/x-icon">
...
<body>
    <a class="logo-wrapper" href="?">
        <img src="/DEVportfolio/logo/dnx-logo_mini_60px.png" alt="Logo">
    ...
    if (isset($_SESSION['user_id'])) {
        $stmt = pdo()->prepare("
            SELECT wallpaper, wallpaper_position, search_engine
            FROM dashboard_settings
            WHERE user_id = :user_id
        ");

        $stmt->execute([':user_id' => $_SESSION['user_id']]);
        $dashboardSettings = $stmt->fetch(PDO::FETCH_ASSOC);

        $_SESSION['search_engine'] = $dashboardSettings['search_engine'];

    } else {
        include 'DEVportfolio/logger.php';
        $wallpaper = "sanabi-wallpaper.jpg";
        $wallpaperPosition = "center";
    }
    echo "<style>";
    echo ".background-overlay {";
    echo "background: url('/dashboard/img/" . $dashboardSettings['wallpaper'] . "') no-repeat " . $dashboardSettings['wallpaper_position'] . "/cover;";
    echo "} </style>";
    ...
    if (isset($_GET['rss'])) {
        ...
        foreach ($feeds as $url) {
            ...
            foreach ($xml->channel->item as $entry) {
                $items[] = [
                    ...
                    'image' => isset($entry->enclosure) ? (string) $entry->enclosure['url'] : ''
                ];
            }
        }
        ...
        <?php
        echo "<img src='/DEVportfolio/logo/dnx-logo_mini_120px.png' 
        style='
            opacity: 0.6;
            position: absolute;
            top: 200px;
        ...
        <div style="position: absolute; z-index: 5; top: 20px; right: 20px; display: flex; align-items: center">
            ...
            <?php if (isset($_SESSION['user_id'])):
                ...
                <?php endif; ?>
                <strong style="margin-left: 12px"><?= htmlspecialchars($_SESSION['username']) ?></strong> <?php
            else: ?>
            ...
            if (!isset($_SESSION['user_id'])): ?>
                <div class="d-block py-3 row gy-1">
                    ...
                    <p>Pro úpravu nastavení se prosím <span class="href-btn" data-bs-toggle="modal"
                            ...
                            přihlaš</span>.</p>
                    <p>Pokud máš propojený účet s Google účtem, můžeš se přes něj i <a href="/auth/google_login.php"
                            class="href-btn">
                            přihlásit</a>.</p>
                </div>
            <?php else: ?>
                <div style="display: flex; align-items: center;">
                    ...
                    <div style="margin-left: auto; display: flex; gap: 0.5rem;">
                        <a href="/auth/logout.php" class="href-btn">Odhlásit se</a>
                        <?php if (isset($_SESSION['user_id']) && empty($_SESSION['google_id'])): ?>
                            <a href="/auth/google_login.php" class="href-btn">Propojit účet Google</a>
                        <?php endif; ?>
                    </div>
                </div>
                <?php
                ...
                $stmt = pdo()->prepare("
                    SELECT id, wallpaper, wallpaper_position, search_engine
                    FROM dashboard_settings
                    WHERE user_id = :user_id
                ");
                ...
                <form method="post" action="/auth/save-settings.php">
                        ...
                        <?php foreach ($wallpapers as $file):
                            ...
                            <div class="wallpaper-thumb">
                                ...
                                <label for="wp-<?= htmlspecialchars($filename) ?>">
                                    <img src="/dashboard/img/<?= htmlspecialchars($filename) ?>"
                        ...
                        if (!isset($_SESSION['user_id'])) {
                            ...
                            foreach ($bookmarks as $i => $bookmark):
                                ...
                                <div class="bookmark-item">
                                    <input type="hidden" name="bookmarks[<?= $i ?>][id]" value="<?= $bookmark['id'] ?? '' ?>">
                    ...
                    ");
                        ...
                        $stmt->execute([':user_id' => $_SESSION['user_id']]);
                        $rssFeeds = $stmt->fetchAll(PDO::FETCH_ASSOC);
                        if (empty($rssFeeds)) {
                            $rssFeeds = [''];
                        }
                        foreach ($rssFeeds as $i => $rss): ?>
                            <div class="rss-item">
                                <input type="hidden" name="<?= $rss['id'] ?>" value="<?= $rss['id'] ?>">
                                <input type="url" name="<?= $rss['feed_url'] ?>" placeholder="RSS URL"
                                    value="<?= htmlspecialchars($rss['feed_url']) ?>" required>
            ...
            <form id="searchForm" method="get" target="_parent" action="<?= htmlspecialchars($searchBase) ?>">
                <div class="search-section"
                    ...
                    <div class="search-logo"
                        ...
                        <img src="/DEVportfolio/logo/dnx-logo_mini_120px.png" alt="Logo">
                    </div>
    ...
    <nav class="bottom-navbar">
        <a href="?dash" class="nav-icon <?php if (isset($_GET['dash'])) {
            echo "active";
        } ?>" title="Dashboard">
            <img src="../DEVportfolio/logo/dnx-logo_mini.ico" alt="Logo">
        </a>
        <a href="?rss" class="nav-icon <?php if (isset($_GET['rss'])) {
            echo "active";
        } ?>" title="RSS feed">📡</a>
        <a href="?set" class="nav-icon <?php if (isset($_GET['set'])) {
            echo "active";
        } ?>" title="Settings">⚙️</a>
    </nav>
    ...
    <?php if (!(isset($_GET['rss']) || isset($_GET['set']))) {
        echo "<script src='/dashboard/js/filter-bookmark.js'></script>";
    } ?>
    <script src="/dashboard/js/scroll-wallpaper.js"></script>
    <!-- Javascript - Obecné -->
    <script src="/dashboard/js/navbar-hide.js"></script>
    <script src="/dashboard/js/logo-animated.js"></script>
    <!-- Javascript pro nastavení -->
    <?php if (isset($_GET['set']) && isset($_SESSION['user_id'])) {
        echo '<script src="/dashboard/js/settings-add.js"></script>
            <script src="/dashboard/js/settings-wallpaper.js"></script>';
    } ?>
